...
const Search = ({setShowSearch}: {setShowSearch: React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  return (
    <div className='absolute h-screen w-full top-0 left-0 bg-black/30 text-center'>
      ...
      <input type="text" id='search' value={query} onChange={e => setQuery(e.target.value)} placeholder='Perfumes/fabrics/t shirt' className='w-[75%] outline-none mt-20 bg-white px-5 py-3 text-black placeholder:text-gray-400 border border-black rounded-3xl mr-3' />
