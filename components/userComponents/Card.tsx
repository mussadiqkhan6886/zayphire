...
const Card = ({images, price, color, gender, category, _id, name, isSale, discountPrice, type, brand, inStock}: Product) => {
  return (
    <div className={`border border-black flex flex-col ${inStock ? "" : "opacity-65"}`}>
      ...
      </Link> : <div className='h-full'>
         <Image src={images[0]} alt={name} width={200} height={300} className='w-full h-full' />
      </div>}
      
      <div className='p-3 py-4 border-t border-black'>
        <div className='leading-2'>
          <h4>{name} <span className='text-red-700'>{inStock ? "" : "Not in stock"}</span></h4>
        ...
        <div className='flex gap-2'>PKR {isSale ? <p className='font-semibold'>{discountPrice} <span className='text-gray-500 font-thin line-through'>{price}</span></p>: price}</div>
      </div>
    </div>  
  )
}
